...
approximates $$V(\b x, t_0)$$, where $$t_0 < t_f$$. Support for the forward
...
variance = 0.2
xs = grid[0]
final_value_grid = tf.math.exp(-xs**2 / (2 * variance)) / tf.math.sqrt(2 * np.pi * variance)
```
...
[^tf_tensordot]: With multiplicatively separable functions, we can avoid
  ...
  ys, xs = grid
  final_value_grid = (
      tf.tensordot(tf.math.exp(-ys**2 / (2 * sigma)),
...
```python
plt.imshow(result_value_grid.numpy(), extent=[x_min, x_max, y_min, y_max], cmap='hot')
plt.show()
```
...
def first_order_coeff_fn(t, grid):
  ...
  # makes the shapes broadcastable to (s_size, v_size, r_size).
  s, v, r = grid
...
Eq.(\ref{space_discretized}) for $$\b V(t + \delta t)$$ given $$\b V(t)$$
...
Eq.(\ref{space_discretized}) with $$\b b = 0$$, and a tridiagonal matrix
$$\hat L$$.
...
Considering that $$\b V$$ in Eq.(\ref{space_discretized}) is a vector, i.e. a
...
conditions, see Eq. (\ref{boundcond}). When discretizing Eq. (\ref{boundcond}),
we approximate the derivative to the second order of accuracy, just like we did
with the PDE terms. The central approximation, Eq. (\ref{deriv_approx_central}),
...
Substituting these two equations into Eq. (\ref{boundcond}) (taking into
account that $$\partial V/\partial\mathbf n = - \partial V/\partial x$$ on the
lower boundary, and $$\partial V/\partial\mathbf n = \partial V/\partial x$$ on
the upper boundary), we get the following discretized versions of it:
...
Substituting Eqs.(\ref{lower_bound_discr}, \ref{upper_bound_discr}), we obtain
for the inner part $$\mathbf V_{inner} = [V_1, ... V_{n-1}]^T$$:

$$\be
\frac {d \mathbf V_{inner}}{dt} = \hat{\tilde L} \mathbf V_{inner} + \mathbf b,
\label {dVdt_with_boundary}
\ee$$

where $$\bea &&\tilde L_{11} = L_{11} + \xi_1 L_{01}, \qquad \tilde L_{12} =
...
*   apply the corrections to the time evolution equation given by Eqs.
    (\ref{bound_corr_first}-\ref{bound_corr_last}),
*   apply the chosen time marching scheme to find the "inner" part of the values
    vector $$\mathbf V_{inner}^{i+1} = \mathbf V_{inner}(t_{i+1})$$ from Eq.
    (\ref{dVdt_with_boundary}) given the values at time $$\mathbf V_{inner}^i$$,
*   find $$V_0$$ and $$V_n$$ from Eqs. (\ref{lower_bound_discr},
    \ref{upper_bound_discr}) at time $$t + \delta t$$, and append them to
    $$\mathbf V_{inner}^{i+1}$$ to get the resulting vector $$\mathbf V^{i+1}$$.

This approach generalizes straighforwardly to the multidimensional case:
$$\b V_{inner}$$ would be the value grid with all the boundaries "trimmed", and
Eqs. (\ref{bound_corr_first}-\ref{bound_corr_last}) would similarly apply to the
items next to the boundary.

### Time marching schemes

Time marchings schemes are algorithms for numerically solving the equation
...
$$\be
\frac{\b V_1 - \b V_0}{\delta t} = \theta(\hat L_0 V_0 + \b b_0) +
...
side of Eq.(\ref{space_discretized}) retains the second order accuracy, while
...
case). The exact solution of Eq. (\ref{space_discretized2}) is then
...
9780128498941. 2015.
https://www.sciencedirect.com/book/9780128498941/numerical-methods-for-partial-differential-equations
...
https://dl.acm.org/citation.cfm?id=2722576
...
Distributed System, vol. 22, pp. 22-32, Jan. 2011.
https://ieeexplore.ieee.org/abstract/document/5445081
...
Analysis Vol. 17, No. 5 (Oct., 1980), pp. 641-655.
https://www.jstor.org/stable/2156665?seq=1
...
Pages 253-263.
https://www.sciencedirect.com/science/article/pii/S009784850200075X

[8] Giles, Michael & Carter, Rebecca. (2005). Convergence analysis of
Crank-Nicolson and Rannacher time-marching. J. Comput. Finance. 9.
https://www.researchgate.net/publication/228524629_Convergence_analysis_of_Crank-Nicolson_and_Rannacher_time-marching
