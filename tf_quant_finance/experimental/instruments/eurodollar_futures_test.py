...
class EurodollarFuturesTest(tf.test.TestCase):
  ...
  def test_edf_correctness(self):
    dtype = np.float64
    notional = 1.
    expiry_date = tff.experimental.dates.convert_to_date_tensor(
        [(2021, 2, 8)])
  ...
  def test_edf_explicit_maturity(self):
    ...
    maturity_date = tff.experimental.dates.convert_to_date_tensor([(2021, 5, 8)
                                                                  ])
    edfuture = tff.experimental.instruments.EurodollarFutures(
  ...
  def test_edf_many(self):
    ...
    maturity_date = tff.experimental.dates.convert_to_date_tensor(
        [(2021, 5, 8), (2021, 5, 8)])
