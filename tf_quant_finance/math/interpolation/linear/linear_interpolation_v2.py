...
"""Linear interpolation method using one hot encodings."""
...
def interpolate(x,
  ...
  not require the function approximated by these knots to be monotonic. This
  function is the same as `linear_interpolation` except it uses tf.one_hot
  ...
  result = linear_interpolation_v2.interpolate(x, x_data, y_data)
  ...
  Args:
    ...
    name: Python str. The name prefixed to the ops created by this function.
      Default value: `None` which maps to 'linear_interpolation_v2'.

  Returns:
    A N-D `Tensor` of real dtype corresponding to the x-values in `x`.
  """
  name = name or 'linear_interpolation_v2'
  with tf.name_scope(name):
    ...
    with tf.control_dependencies(control_deps):
      ...
      upper_encoding = tf.one_hot(capped_upper_indices, x_data_size,
                                  dtype=dtype)
      def get_slice(x, e):
        return tf.math.reduce_sum(tf.expand_dims(x, axis=-2) * e, axis=-1)
      ...
      return interpolated if batch_shape else tf.squeeze(interpolated, 0)
