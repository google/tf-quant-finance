...
class AsianPriceTest(parameterized.TestCase, tf.test.TestCase):
  ...
  def test_option_prices(self):
    """Tests for methods for the asian pricing module. Results come from
    'Implementing Derivatives Model' by Clewlow, Strickland p.118-123, via the
    QuantLib test suite
    """
    ...
    def set_up_pricer(expiries, watch_params=False):
        def price_asian_options(strikes, spot, sigma):
            ...
            paths = process.sample_paths(
                sampling_times, num_samples=num_samples,
                initial_state=log_spot, 
                watch_params=watch_params_list,
                random_type=tff.math.random.RandomType.PSEUDO_ANTITHETIC,
                seed=43,
                time_step=dt)
            prices = (tf.exp(-tf.expand_dims(discount_rates * expiries, axis=-1))
                      * tf.reduce_mean(tf.nn.relu(tf.math.exp(tf.reduce_mean(paths, [1])) - strikes), [0]))
            return prices
