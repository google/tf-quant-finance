...
class AveragingType(enum.Enum):
  ...
  * `GEOMETRIC`: C = ( \prod_1^n S(t_i) ) ^ {\frac{1}{n}}
  * `ARITHMETIC`: C = \frac{1}{n} \sum_1^n S(t_i)
  """
  GEOMETRIC = 1
  ARITHMETIC = 2
...
def asian_option_price(*,
  ...
  TODO: support volatility termstructures
  For now we provide a single volatility for each option. A simple extension of
  Black-Scholes provides a termstructure of volatilities that obeys non-negative
  forward variance. We could also use this methodology under that model.
  ...
  # [ 0.        , 0.        , 0.52833763, 0.99555802, 1.91452834]
  ```
  ...
  with tf.name_scope(name or 'asian_option_price'):
    ...
    # F \to \Bigl( \prod_{i=1}^n F_i \Bigr)^{\frac{1}{n}} e^{\frac{1}{2} \sigma^2 (t1 - t2)}
    #
    ...
    return vanilla_prices.option_price(volatilities=effective_volatilities,
                                       strikes=strikes,
                                       expiries=expiries,
