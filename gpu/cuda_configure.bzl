...
def _normalize_include_path(repository_ctx, path):
    """Normalizes include paths before writing them to the crosstool.
        ...
        path is returned.
        If path points outside the 'crosstool' folder, an absolute path is
        returned.
        """
...
def _host_compiler_includes(repository_ctx, cc):
    """Generates the cxx_builtin_include_directory entries for gcc inc dirs.
        ...
        Returns:
          A string containing the cxx_builtin_include_directory for each of the
          gcc
...
def find_cuda_define(repository_ctx, header_dir, header_file, define):
    """Returns the value of a #define in a header file.
        ...
        Greps through a header file and returns the value of the specified
        #define.
...
def _find_cupti_header_dir(repository_ctx, cuda_config):
    """Returns the path to the directory containing cupti.h
        ...
        On most systems, the cupti library is not installed in the same directory
        as
        the other CUDA libraries but rather in a special extras/CUPTI directory.
        Args:
          repository_ctx: The repository context.
    ...
    for relative_path in CUPTI_HEADER_PATHS:
        if repository_ctx.path("%s/%scupti.h" %
                               (cuda_toolkit_path, relative_path)).exists:
            return ("%s/%s" % (cuda_toolkit_path, relative_path))[:-1]
    auto_configure_fail(
        "Cannot find cupti.h under %s" %
        ", ".join([cuda_toolkit_path + "/" + s for s in CUPTI_HEADER_PATHS]),
    )
...
def _find_cuda_include_path(repository_ctx, cuda_config):
    ...
    for relative_path in CUDA_INCLUDE_PATHS:
        if repository_ctx.path("%s/%scuda.h" %
                               (cuda_toolkit_path, relative_path)).exists:
            return ("%s/%s" % (cuda_toolkit_path, relative_path))[:-1]
    ...
    """Detects and returns information about the CUDA installation on the system.
        ...
        Returns:
          A struct containing the following fields:
            ...
            compute_capabilities: A list of the system's CUDA compute
            capabilities.
            cpu_value: The name of the host operating system.
        """
...
def _create_dummy_repository(repository_ctx):
    ...
    repository_ctx.file("cuda/cuda/lib/%s" %
                        lib_name("cudart_static", cpu_value))
...
def _create_local_cuda_repository(repository_ctx):
    ...
    wrapper_defines = {
        ...
        "%{cuda_compute_capabilities}": ", ".join(["\"%s\"" % c for c in cuda_config.compute_capabilities]),
