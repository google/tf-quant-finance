...
* <b>`x_data`</b>: A `Tensor` of real `dtype` and shape `[num_x_data_points]`.
...
* <b>`x_values`</b>: Real-valued `Tensor` of rank 1. Defines the x-coordinates at
  which the interpolation should be performed.
* <b>`y_values`</b>: Rank 1 `Tensor` of the same `dtype` as `x_values`. Defines the
