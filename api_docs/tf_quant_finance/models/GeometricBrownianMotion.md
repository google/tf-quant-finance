...
* <b>`mu`</b>: Scalar real `Tensor` or an instance of batch-free left-continuous
  `PiecewiseConstantFunc`. Corresponds to the mean of the Ito process.
* <b>`sigma`</b>: Scalar real `Tensor` or an instance of batch-free left-continuous
...
* <b>`ValueError`</b>: If `normal_draws` is supplied and `1` is not `1`.
